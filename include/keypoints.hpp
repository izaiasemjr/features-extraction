...
static const string KP_SUSAN                 = "";
static const string KP_UNIFORM_SAMPLING      = "UniformSampling";
static const string KP_LANDMARKS_MANUALY     = "landmarksManually";
...
public:
  ...
  explicit Keypoints(const string kp_type, string filename);


  // Detect
  void compute(const PointCloudRGB::Ptr& cloud, PointCloudRGB::Ptr& cloud_keypoints);
  void getKeypointsFromFile(PointCloudXYZ::Ptr& cloud_keypoints);
...
Keypoints::Keypoints(const string kp_type,string filename) :
    kp_type_(kp_type),  filename_(filename){}
...
/** \brief Detects the keypoints of the input cloud
  ...
  * \param Stores the indices of the detected keypoints on the input cloud
  */
void Keypoints::getKeypointsFromFile(PointCloudXYZ::Ptr& cloud_keypoints)
{
  // open key points frm file pcd
  if (kp_type_ == KP_LANDMARKS_MANUALY){
      openCloudPCD(filename_.c_str(),cloud_keypoints);
      return;
  }
  else {
    cout<<"The method called not exist, call a valid method"<<endl;
    exit(1);
  }
}
